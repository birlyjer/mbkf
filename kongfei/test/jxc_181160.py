...
monitor_181160_drugs = cr.fetchall()

drug_code = [x[0] for x in monitor_181160_drugs]
drug_name = [x[1] for x in monitor_181160_drugs]
...
# 期初库存
def beginning_num(i):
    beginning_num = "select actualamount from ce_invent_reg  where institutioncode=181160 and nationaldrugcode=%s and to_char(checktime,'YYYY-MM-DD hh24:mi:ss')=to_char((select max(checktime) from CE_INVENT_REG where institutioncode=181160 and nationaldrugcode=%s),'YYYY-MM-DD hh24:mi:ss' ) " % \
...
# 当期入库
def in_num(i):
    in_num = "select sum(drugamount*(select usageunitamountperpackage from ce_inventorymonitor_items  where  institutioncode=181160 and nationaldrugcode =%s)) from ce_inventory_in_181160 where nationaldrugcode=%s and to_char(inputtime,'YYYY-MM-DD hh24:mi:ss')>=to_char((select max(checktime) from CE_INVENT_REG where institutioncode=181160),'YYYY-MM-DD hh24:mi:ss')" % \
             (drug_code[i], drug_code[i])
    cr.execute(in_num)
    rs = cr.fetchall()
    if rs != [(None,)]:
...
# 当期出库
def out_num(i):
    out_num = "SELECT   sum(cnt) from ce_inventory_out_181160 where national_drug_code=%s  and to_char(pay_date,'YYYY-MM-DD hh24:mi:ss')>=to_char((select max(checktime) from CE_INVENT_REG where institutioncode=181160),'YYYY-MM-DD hh24:mi:ss' )" % \
              drug_code[i]
    cr.execute(out_num)
    rs = cr.fetchall()
    if rs != [(None,)]:
...
# his实际库存
def actual_num(i):
    actual_num = "select sum(package_cnt)his实际库存最新 from ce_inventory_account_181160 where national_drug_code=%s and drug_store_name !='住院部药房' and account_date = (select max(account_date) from ce_inventory_account_181160 where national_drug_code='86903942000995' )" % \
                 drug_code[i]
...
# 销售金额
def sailes_sum(i):
    sailes_sum = "select sum(total_price)  from ce_inventory_out_181160 where national_drug_code=%s" % drug_code[i]
...
# 购买金额
def purchase_sum(i):
    purchase_sum = "select sum(totalvalue) from ce_inventory_in_181160 where nationaldrugcode =%s" % drug_code[i]
...
for i in range(len(drug_code)):
    ...
    jxc_list_value = [beginning_num(i), in_num(i), out_num(i), actual_num(i), '%.2f%%' % (pcbfb(i) * 100), pcl,
                      sailes_sum(i), purchase_sum(i)]
