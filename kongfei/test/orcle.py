...
# 当期出库
def out_num(i):
    out_num = "SELECT   sum(cnt) from ce_inventory_out_181220 where national_drug_code=%s  and to_char(pay_date,'YYYY-MM-DD hh24:mi:ss')>=to_char((select max(checktime) from CE_INVENT_REG where institutioncode=181220),'YYYY-MM-DD hh24:mi:ss' )" % \
              drug_code[i]
    cr.execute(out_num)
    rs = cr.fetchall()
    if rs !=[(None,)]:
...
# his实际库存
def actual_num(i):
    actual_num = "select sum(package_cnt)his实际库存最新 from ce_inventory_account_181220 where national_drug_code=%s and drug_store_name !='住院部药房' and account_date = (select max(account_date) from ce_inventory_account_181220 where national_drug_code='86903942000995' )" % \
                 drug_code[i]
